...
public class McpAuditService {
    ...
    public void logOperation(String operationType, String toolName, boolean success, String message, long startTime) {
        try {
            ...
            // Capture PDF job context for traceability
            String jobIdStr = McpSecurityContext.getJobId();
            Long jobId = null;
            if (jobIdStr != null) {
                try {
                    jobId = Long.parseLong(jobIdStr);
                } catch (NumberFormatException e) {
                    log.warn("Invalid job ID in context: {}", jobIdStr);
                }
            }
            ...
            McpAuditLog auditLog = McpAuditLog.builder()
                    ...
                    // PDF Job Context
                    .jobId(jobId)
                    .sourceFilename(McpSecurityContext.getJobSourceFilename())
                    .s3Bucket(McpSecurityContext.getJobS3Bucket())
                    .s3ObjectKey(McpSecurityContext.getJobS3ObjectKey())
                    .build();
