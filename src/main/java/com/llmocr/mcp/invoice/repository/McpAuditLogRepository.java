...
public interface McpAuditLogRepository extends JpaRepository<McpAuditLog, Long> {
    ...
    /**
     * Find audit logs for a specific source filename (PDF file)
     * Used to show what MCP tools were called during invoice processing
     */
    List<McpAuditLog> findByTenantIdAndSourceFilenameOrderByCreatedAtAsc(
            String tenantId, String sourceFilename);

    /**
     * Find audit logs by job ID
     */
    List<McpAuditLog> findByTenantIdAndJobIdOrderByCreatedAtAsc(
