        ...
        try {
            ...
            List<Map<String, Object>> tools = toolCallbacks.stream()
                .map(callback -> {
                    ...
                    // Convert inputSchema string to JSON object (fixes client-side warnings)
                    String schemaStr = callback.getToolDefinition().inputSchema();
                    try {
                        JsonNode schemaNode = objectMapper.readTree(schemaStr);
                        tool.put("inputSchema", schemaNode);
                    } catch (Exception e) {
                        // Fallback to string if parsing fails
                        tool.put("inputSchema", schemaStr);
                    }
