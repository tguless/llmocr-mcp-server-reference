...
public class McpSecurityContext {
    ...
    /**
     * Get job-specific context from request headers (auto-injected by backend, not AI-controlled)
     */
    public static String getJobSourceFilename() {
        try {
            HttpServletRequest request = getCurrentRequest();
            if (request != null) {
                return (String) request.getAttribute("mcpSourceFilename");
            }
        } catch (Exception e) {
            log.debug("Could not get source filename from request context: {}", e.getMessage());
        }
        return null;
    }

    public static String getJobS3Bucket() {
        try {
            HttpServletRequest request = getCurrentRequest();
            if (request != null) {
                return (String) request.getAttribute("mcpS3Bucket");
            }
        } catch (Exception e) {
            log.debug("Could not get S3 bucket from request context: {}", e.getMessage());
        }
        return null;
    }

    public static String getJobS3ObjectKey() {
        try {
            HttpServletRequest request = getCurrentRequest();
            if (request != null) {
                return (String) request.getAttribute("mcpS3ObjectKey");
            }
        } catch (Exception e) {
            log.debug("Could not get S3 object key from request context: {}", e.getMessage());
        }
        return null;
    }
    ...
    public static String getJobId() {
        try {
            HttpServletRequest request = getCurrentRequest();
            if (request != null) {
                return (String) request.getAttribute("mcpJobId");
            }
        } catch (Exception e) {
            log.debug("Could not get job ID from request context: {}", e.getMessage());
        }
        return null;
    }

    public static String getSourceExternalId() {
        try {
            HttpServletRequest request = getCurrentRequest();
            if (request != null) {
                return (String) request.getAttribute("mcpSourceExternalId");
            }
        } catch (Exception e) {
            log.debug("Could not get source external ID from request context: {}", e.getMessage());
        }
        return null;
    }

    public static String getSourceMetaInvoiceRef() {
        try {
            HttpServletRequest request = getCurrentRequest();
            if (request != null) {
                return (String) request.getAttribute("mcpSourceMetaInvoiceRef");
            }
        } catch (Exception e) {
            log.debug("Could not get source metadata invoiceRef from request context: {}", e.getMessage());
        }
        return null;
    }
